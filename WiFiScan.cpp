...
extern "C" int ieee80211_raw_frame_sanity_checks(int32_t arg, int32_t arg2, int32_t arg3)
{
  if (arg == 31337)
    return 1;
  else
...
class bluetoothScanAllCallback : public BLEAdvertisedDeviceCallbacks
{
  ...
  void onResult(BLEAdvertisedDevice *advertisedDevice)
  {
    ...
    if (buf >= 0)
    {
      // display_string.concat(text_tables4[0]);
...
{
  if (ieee80211_raw_frame_sanity_checks(31337, 0, 0) == 1)
...
void WiFiScan::initWiFi(uint8_t scan_mode)
{
  ...
  if (scan_mode != WIFI_SCAN_OFF)
  {
    ...
    //  this->force_pmkid = settings_obj.loadSetting<bool>(text_tables4[5]);
    //  this->force_probe = settings_obj.loadSetting<bool>(text_tables4[6]);
    //  this->save_pcap = settings_obj.loadSetting<bool>(text_tables4[7]);
...
void WiFiScan::StartScan(uint8_t scan_mode, uint16_t color)
{
  ...
  /* else if (scan_mode == WIFI_ATTACK_BEACON_LIST)
     this->startWiFiAttacks(scan_mode, color, text_tables1[50]);
   else if (scan_mode == WIFI_ATTACK_BEACON_SPAM)
     this->startWiFiAttacks(scan_mode, color, text_tables1[51]);
   else if (scan_mode == WIFI_ATTACK_RICK_ROLL)
     this->startWiFiAttacks(scan_mode, color, text_tables1[52]);
   else if (scan_mode == WIFI_ATTACK_AUTH)
     this->startWiFiAttacks(scan_mode, color, text_tables4[7]);
   else if (scan_mode == WIFI_ATTACK_DEAUTH)
     this->startWiFiAttacks(scan_mode, color, text_tables4[8]);
   else if (scan_mode == WIFI_ATTACK_DEAUTH_MANUAL)
     this->startWiFiAttacks(scan_mode, color, text_tables4[8]);
   else if (scan_mode == WIFI_ATTACK_DEAUTH_TARGETED)
     this->startWiFiAttacks(scan_mode, color, text_tables4[47]);*/
...
void WiFiScan::RunAPScan(uint8_t scan_mode, uint16_t color)
{
  ...
  // Serial.println(text_tables4[9] + (String)access_points->size());
#ifdef HAS_SCREEN
  display_obj.TOP_FIXED_AREA_2 = 48;
  display_obj.tteBar = true;
  display_obj.print_delay_1 = 15;
...
{
  extern WiFiScan wifi_scan_obj;
  bool send_deauth = true; // settings_obj.loadSetting<bool>(text_tables4[5]);
...
{
  bool save_packet = true; // settings_obj.loadSetting<bool>(text_tables4[7]);
  if (save_packet)
  {
#ifdef WRITE_PACKETS_SERIAL
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  else if ((currentScanMode == WIFI_ATTACK_MIMIC))
  {
    ...
    {
      if (access_points->get(i).selected)
        this->broadcastCustomBeacon(currentTime, ssid{access_points->get(i).essid, random(1, 12), {random(256), random(256), random(256), random(256), random(256), random(256)}});
    }
